...
@triton.jit
def _residual_add_layernorm_fwd_kernel(Residual, SublayerOut, OutNorm, OutSum,
    Weight, Bias, stride, M, N, eps: tl.constexpr, BLOCK_SIZE: tl.constexpr, STORE_FP16: tl.constexpr):
...
def residual_add_layernorm(residual: torch.Tensor, sublayer_out: torch.Tensor,
    weight: torch.Tensor, bias: torch.Tensor, eps: float = 1e-5) -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    num_programs = (M + 1) // 2
    _residual_add_layernorm_fwd_kernel[(num_programs,)](residual, sublayer_out, out_norm, out_sum,
        weight, bias, stride=stride, M=M, N=N, eps=eps, BLOCK_SIZE=BLOCK_SIZE, STORE_FP16=store_fp16,
